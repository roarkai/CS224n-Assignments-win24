...
Options:
    ...
    --verbose                               recording beamsearch process [default: False]
"""
import math
import sys
import pickle
...
def decode(args: Dict[str, str]):
    ...
    hypotheses, bs_record = beam_search(model, test_data_src,
    ...
    if args['--verbose']:
        with open(args['OUTPUT_FILE'], 'w') as f:
            ...
            f.write(f'\n\n\nthe following is half sentences during the beam search process:')
            for t, i_step_cands in enumerate(bs_record):
                f.write(f'\n{t} step result: ' + '*' * 20 + '\n')
                for k in i_step_cands:
                    half_sent = ''.join(k).replace('_', ' ')
                    f.write(half_sent + '\n')
    else:
        with open(args['OUTPUT_FILE'], 'w') as f:
            for src_sent, hyps in zip(test_data_src, hypotheses):
...
def beam_search(model: NMT, test_data_src: List[List[str]], beam_size: int, max_decoding_time_step: int, verbose=False) -> List[List[Hypothesis]]:
    ...
    with torch.no_grad():
        for src_sent in tqdm(test_data_src, desc='Decoding', file=sys.stdout):
            example_hyps, bs_record = model.beam_search(src_sent, beam_size=beam_size, 
    ...
    if verbose:
        return hypotheses, bs_record # only take bs_record for one sentence
    else:
