...
if [ "$1" = "train" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 python run.py decode model.bin ./zh_en_data/rk.zh outputs/rk_outputs.txt --cuda
elif [ "$1" = "rk_beam_search_verbose" ]; then
    CUDA_VISIBLE_DEVICES=0 python run.py decode model.bin ./zh_en_data/rk.zh outputs/rk_verbose_outputs.txt --cuda --verbose
else
