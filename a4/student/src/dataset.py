...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ### YOUR CODE HERE ###
        doc = self.data[idx]
        trunc_len = random.randint(4, int(self.block_size * 7 / 8)) # length of the truncated document
        trunc_doc = doc[:trunc_len]

        # randomly choosing the length of masked content
        # plan1: vanilla dev accuracy = 12%
        # mask_len = random.randint(1, int(0.5 * trunc_len))   

        # plan2: vanilla dev accuracy = 27%, rope dev accuracy = 20%
        # mask_len = int(np.random.beta(1, 3) * trunc_len)     

        # plan3: vanilla dev accuracy = 21%, rope dev accuracy = 25.8%
        avg_len = trunc_len // 4
        mask_len = avg_len + random.randint(-avg_len // 2, avg_len // 2)  

        mask_start = random.randint(0, trunc_len - mask_len)
        mask_stop = mask_start + mask_len
        
        prefix = trunc_doc[:mask_start]
        masked_content = trunc_doc[mask_start:mask_stop]
        suffix = trunc_doc[mask_stop:]

        # rearrange word pieces
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content \
                        + self.PAD_CHAR * (self.block_size - len(trunc_doc) - 1)
        
        # construct the input and output
        x = masked_string[:-1]
        y = masked_string[1:]
        # print(f'\nstart={mask_start}, step={mask_len}, stop={mask_stop}, doc len={len(trunc_doc)}, {len(masked_string)}')

        # encode the input and output strings as long tensor
