...
if args.function == 'pretrain':
    ...
    # setting Hyperparameters for finetuning
    train_conf = trainer.TrainerConfig(max_epochs=650,
                                       batch_size=128,
                                       learning_rate=args.pretrain_lr,
                                       lr_decay=True,
                                       warmup_tokens=512*20,
    ...
    # training
    tr_trainer = trainer.Trainer(model, pretrain_dataset, None, train_conf)
    tr_trainer.train()
    ...
    # setting Hyperparameters for finetuning
    ft_conf = trainer.TrainerConfig(max_epochs=75,
                                      batch_size = 256,
                                      learning_rate=args.finetune_lr,
                                      lr_decay=True,
                                      warmup_tokens=512*20,
    ...
    if args.reading_params_path is not None:
        # load pretrained model parameters into the model
        model.load_state_dict(torch.load(args.reading_params_path))

        # change max_epoch setting when there is a pretrained model
        ft_conf.max_epochs = 10  # if the finetune epoch is 75, the accuracy would go down a lot

    # prepare data for finetuning
    name_dataset = dataset.NameDataset(pretraining_dataset=pretrain_dataset, 
                                        data=open(args.finetune_corpus_path, encoding='utf-8').read())
        
    # finetuning
    ft_trainer = trainer.Trainer(model, name_dataset, None, ft_conf)
    ft_trainer.train()
